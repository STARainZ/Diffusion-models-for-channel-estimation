    ...
    def reverse_step(self, x_t: torch.Tensor, t: int, *, add_random: bool = False):
        ...
        if self.reverse_method == 'reverse_mean':
            ...
            if add_random:
                ...
                noise = np.sqrt(0.01) * self.noise_multiplier * torch.randn_like(x_t) if t > 0 else 0.  # don't add noise in last step
        ...
        return {"sample": x_pred, "x0": (x_0 if self.reverse_method == 'ground_truth' else None), "eps_t": pred_noise}
